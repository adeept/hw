...
def robotCtrl(command_input, response):
    ...
    #     test = 1
    #     while True:
    #         if 'home' == command_input:
    #             if test == 1:
    #                 move.move(speed_set, 'forward', 'no', rad)#forward
    #                 print("Forward")
    #                 test = 0
    #             time_end = time.time()
    #             if mark == 0 and time_end-time_start >= 1.5:  # 上一个动作持续1.5s.
    #                 move.move(speed_set, 'no', 'no', rad)# stop
    #                 print("stop")
    #                 move.move(speed_set, 'backward', 'no', rad)# backward
    #                 print("backward")
    #                 time_start = time_end
    #                 time_end = time.time()
    #                 mark = 1
    #             if mark == 1 and time_end-time_start >= 1.5:  # 上一个动作持续1.5s.
    #                 move.move(speed_set, 'no', 'no', rad)# stop
    #                 print("stop")
    #                 move.move(speed_set, 'forward', 'no', rad)# forward
    #                 print("forward")
...
def Back_and_forth():
    ...
    while True:
        ...
        if test_mark == 1:
            if test == 1:
                time_start = time.time()
                move.move(motor_speed, 'forward', 'no', rad) # forward
                print("Forward")
                test = 0
            time_end = time.time()
            if mark == 0 and time_end-time_start >= run_time:  # 上一个动作持续1.5s.
                move.move(motor_speed, 'no', 'no', rad)# stop
                time.sleep(0.2)
                print("stop")
                move.move(motor_speed, 'backward', 'no', rad)# backward
                print("backward")
                time_start = time_end
                time_end = time.time()
                mark = 1
            if mark == 1 and time_end-time_start >= run_time:  # 上一个动作持续1.5s.
                move.move(motor_speed, 'no', 'no', rad)# stop
                time.sleep(0.2)
                print("stop")
                move.move(motor_speed, 'forward', 'no', rad)# forward
...
if __name__ == '__main__':
    ...
    testNC_threading=threading.Thread(target=Back_and_forth)
    testNC_threading.setDaemon(False)
    testNC_threading.start()  
